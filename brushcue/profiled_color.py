...
class ProfiledColor(Object):
    ...
    def new_lmsa(lmsa) -> ProfiledColor:
        ...
        Returns:
            Graph: A graph node producing a ProfiledColor.
        """
        lmsa_parsed = input_parsers.parse_graph(lmsa)
        result = _internal.profiled_color_from_l_m_s_a_internal(lmsa_parsed)
        return ProfiledColor(result)
    ...
    def from_wavelength_nm(wavelength_nm) -> ProfiledColor:
        ...
        Args:
            wavelength_nm: Graph of Float

        Returns:
            Graph: A graph node producing a ProfiledColor.
        """
        wavelength_nm_parsed = input_parsers.parse_float_graph(wavelength_nm)
        result = _internal.profiled_color_from_wavelength_nm_internal(wavelength_nm_parsed)
        return ProfiledColor(result)
    ...
    def to_lmsa(self) -> l_m_s_a.LMSA:
        """Profiled Color to LMS with Alpha

        Converts a profiled color to LMS channels with alpha.

        Returns:
            Graph: A graph node producing a LMSA.
        """
        profiled_color_parsed = input_parsers.parse_graph(self)
        result = _internal.profiled_color_to_l_m_s_a_internal(profiled_color_parsed)
